...
char* CNWNXTMIX::OnRequest(char *gameObject, char* Request, char* Parameters)
{
	Log(1, "Request: \"%s\"\n", Request);
	Log(1, "Params:  \"%s\"\n", Parameters);
...
BOOL CNWNXTMIX::OnCreate(const char* LogDir)
{
	char log[MAX_PATH];
	sprintf (log, "%s\\nwnx_tmix.txt", LogDir);
...
void CNWNXTMIX::WriteLogHeader()
{
	...
	fprintf(m_fFile, "Updated for VS2019");
}

void CNWNXTMIX::GetTMILimit(char *value) {
	int lim = 0;
...
void CNWNXTMIX::SetTMILimit(char *value) {
	int lim = atoi(value);
